...
※本專案是我在緯育在職進修期間的小組專題成果。

---

### 為什麼要做這個專案？

國際企業(Apple、Google、Microsoft等)皆積極推行 ESG 永續發展，並嚴格限制供應鏈碳排放；台灣金管會亦於 2025 年起，要求全體上市櫃公司均應申報永續報告書，企業永續發展不再是選擇題，而是必答題。
...
* **自動化爬蟲 pipeline：** 公開資訊觀測站的 PDF 多為外連到企業官方網站，格式雜亂讓我們一度在初期就卡住，我們轉而分析其他服務平台，挖掘其背後的共同點，成功設計了自動化爬蟲功能。
* **多執行緒與平行處理：** 新聞搜尋受限於GNews API的限制，實際上是最耗時的階段，我們改用多執行緒同時抓取多個議題的資料，並把「文字雲生成」跟「AI 分析」拆開同步執行，不再讓系統傻傻排隊。
